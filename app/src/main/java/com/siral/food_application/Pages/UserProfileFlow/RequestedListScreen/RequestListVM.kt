...
class RequestListVM : RequestListModel() {

    init {
        database = Firebase.database.getReference("receiver_list")
    ...
    fun getRequestedDonations() {
                    ...
                    .addValueEventListener(object : ValueEventListener {
                        override fun onDataChange(snapshot: DataSnapshot) {
                            if(snapshot.exists()) {
                                ...
                                list = list.filter { it.donationStatus == DonationStatus.Donated.key } as MutableList<DonationBO>
                                getData(list)
                                loading = false
                            }
                            else
     ...
     fun getData(list: MutableList<DonationBO>)
     {
         try{
             if (list.size >0) {
             list.forEach {
                 userDatabase.child(it.donationId)
                     .addListenerForSingleValueEvent(object : ValueEventListener {
                         override fun onDataChange(dataSnapshot: DataSnapshot) {
                             ...
                             requestList.postValue(list2)
                             loading = false
                             Log.d("requestListScreen", list2.toString())
                         }
    ...
    fun changeRequestStatus(request: DonationRequestBO, navController: NavHostController) {
        try {
            val updates = mapOf(
                "${request.donationBO.requestId}/donationStatus" to DonationStatus.Accepted.key)
            Log.d("errorfind","${request.donationBO.donationId}/donationStatus")
        ...
        {
            Log.d("RequestListScreen",e.toString())
        }
    }
}